...
from .agent import ManualControlAgent, RandomAgent, RuleBasedAgent, Agent, StrAgent, StrAgent_m, StrAgent_s, StrAgent_b, StrAgent_p, StrAgent_w, ComboAgent, EnsembleAgent, SoftAgent
    ...
    while not env.terminal():
        ...
        for p, agent in agents.items():
            if p in turn_players:
                agent.play_card(env) #検証用の改造箇所、人間は任意にアクションができるようにする
                while p in turn_players:
                    actions = agent.action(env, p, show=show)
                    env.play(actions, p)
                    p =env.turn()
...
def build_agent(raw, env=None):
    if raw == 'random':
        ...
        return ManualControlAgent()
    else:
        model = load_model(raw, env.net())
        agent = Agent(model)
        return agent
    return None
...
class Evaluator:
    ...
    def execute(self, models, args):
        ...
        agents = {}
        "相手3人全員がopponentsからランダムで決まるように変更"
        for p, model in models.items():
            if model is None:
                agents[p] = build_agent(random.choice(opponents), self.env)
            else:
...
def eval_process_mp_child(agents, critic, env_args, index, in_queue, out_queue, seed, show=False):
    ...
    while True:
        ...
        if isinstance(list(agent_map.values())[0], NetworkAgent):
            ...
            outcome , cards= exec_match(env, agent_map, critic, show=show, game_args=game_args)
            #print(agent_ids,cards)
            '''for i in range(len(agents)):
                if agent_ids[i]==0:
                    with open('/Users/tomoya/HandyRL3/handyrl/attack_log.csv', 'a', newline="") as f:
                        writer = csv.writer(f)
                        #set1
                        #writer.writerow([cards[i][0],cards[i][1],cards[i][2],cards[i][3],cards[i][4],cards[i][5],cards[i][6],cards[i][7],cards[i][9],cards[i][11],cards[i][12],cards[i][14],cards[i][15],cards[i][16],cards[i][20],cards[i][23],cards[i][27]])
                        #set2
                        #writer.writerow([cards[i][0],cards[i][1],cards[i][2],cards[i][3],cards[i][4],cards[i][5],cards[i][6],cards[i][8],cards[i][11],cards[i][18],cards[i][21],cards[i][22],cards[i][24],cards[i][28],cards[i][30],cards[i][31],cards[i][32]])
                        #set3
                        #writer.writerow([cards[i][0],cards[i][1],cards[i][2],cards[i][3],cards[i][4],cards[i][5],cards[i][6],cards[i][7],cards[i][9],cards[i][11],cards[i][12],cards[i][14],cards[i][20],cards[i][21],cards[i][27],cards[i][30],cards[i][31]])
                        #set4
                        #writer.writerow([cards[i][0],cards[i][1],cards[i][2],cards[i][3],cards[i][4],cards[i][5],cards[i][6],cards[i][10],cards[i][13],cards[i][14],cards[i][17],cards[i][19],cards[i][21],cards[i][25],cards[i][26],cards[i][28],cards[i][29]])
                        #set5
                        #writer.writerow([cards[i][0],cards[i][1],cards[i][2],cards[i][3],cards[i][4],cards[i][5],cards[i][6],cards[i][7],cards[i][13],cards[i][16],cards[i][18],cards[i][19],cards[i][20],cards[i][22],cards[i][25],cards[i][28],cards[i][29]])
                        #set6
                        writer.writerow([cards[i][0],cards[i][1],cards[i][2],cards[i][3],cards[i][4],cards[i][5],cards[i][6],cards[i][7],cards[i][9],cards[i][12],cards[i][15],cards[i][17],cards[i][19],cards[i][23],cards[i][27],cards[i][31],cards[i][32]])
                        #set7
                        #writer.writerow([cards[i][0],cards[i][1],cards[i][2],cards[i][3],cards[i][4],cards[i][5],cards[i][6],cards[i][8],cards[i][10],cards[i][12],cards[i][13],cards[i][17],cards[i][18],cards[i][21],cards[i][26],cards[i][27],cards[i][30]])
                        '''
...
def eval_main(args, argv):
    ...
    env = make_env(env_args)
    '''with open('/Users/tomoya/HandyRL3/handyrl/attack_log.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['銅貨','銀貨','金貨','屋敷','公領','属州','呪い','地下貯蔵庫','堀','工房','商人','村','民兵','役人','鉱山','盗賊','魔女'])'''
    ...
    opponents = model_paths[1:] if len(model_paths) > 1 else 'random'
    agents = [main_agent] + [resolve_agent(opponents[_]) for _ in range(len(env.players()) - 1)]
