        ...
        for t in range(batch_shape[1]):
            ...
            if t < args['burn_in_steps']:
                ...
                if not model.training:
                    model.train()
                hidden_ = map_r(hidden_, lambda h: h.unsqueeze(0))
                #print('hid3 ' + str(hidden_[0].shape))
                #print('obs ' + str(obs['action'].shape)+'  '+str(obs['state'].shape))
                outputs_ = model(obs, hidden_)
            #print('out ' + str(outputs_['policy'].shape))
            #print('val ' + str(outputs_['value'].shape))
            #print('hid ' + str(outputs_['hidden'][0].shape))
            #print('bs ' + str(batch_shape))
            #outputs_ = map_r(outputs_, lambda o: o.unflatten(0, (batch_shape[0], batch_shape[2])))  # (..., B, P or 1, ...)
            #outputs_['policy'] = outputs_['policy'].unflatten(0, (batch_shape[0], batch_shape[2]))
            #outputs_['value'] = outputs_['value'].unflatten(0, (batch_shape[0], batch_shape[2]))
            for k, o in outputs_.items():
                if k == 'hidden':
                    next_hidden = o
                else:
                    outputs[k] = outputs.get(k, []) + [o]
            next_hidden = map_r(next_hidden, lambda h: h.squeeze(0))
            next_hidden = map_r(next_hidden, lambda h: h.unsqueeze(1))
            x = tuple([
            torch.ones(1,4,1),
            torch.ones(1,4,1)
            ])
            x = to_gpu(x)
            next_hidden = bimap_r(next_hidden, x, lambda h, m: h * m)
            #print('hid '+str(hidden[0].shape))
            #print('nhid '+str(next_hidden[0].shape))
            #print('omas '+str(omask[0].shape))
    ...
    def update(self):
        ...
        if self.model_epoch not in self.results:
            ...
            def output_wp(name, results):
                ...
                #if name == 'total':wandb.log({"win rate": (mean + 1) / 2})
